...
Almohawell::Package::Deb - an object that represents a deb package

=cut

package Almohawell::Package::Deb;
use strict;
use base qw(Almohawell::Package);
...
sub install {
	...
	my $v=$Almohawell::Package::verbose;
	$Almohawell::Package::verbose=2;
	$this->do("dpkg", "--no-force-overwrite", "-i", $deb)
		or die "Unable to install";
	$Almohawell::Package::verbose=$v;
}
...
sub prep {
	...
	print OUT "\n";
	if (! $this->without) {
	print OUT "  * Converted from .".$this->origformat." format to .deb by almohawell version $Almohawell::Version\n"
	};
	print OUT "  \n";
	...
	if (defined $this->rgroup) {
	print OUT "Section: ".$this->rgroup."\n";
	}
	else {
	print OUT "Section: almohawell\n";
	}
	print OUT "Priority: extra\n";
	if (defined $this->maintainer) {
	print OUT "Maintainer: ".$this->maintainer."\n";
	}
	else {
	...
	if (defined $this->depends) {
		...
		print OUT "Depends: ".join(", ", "\${shlibs:Depends}", $this->depends, $this->requires)."\n";
		}
		else {
		...
		if (defined $this->requires) {
		print OUT "Depends: ".join(", ", "\${shlibs:Depends}", $this->requires)."\n";
		}
		else {
	...
	if (! $this->without) {
	print OUT "This package was debianized by the almohawell program by converting\n"
	};
	print OUT "a binary .".$this->origformat." package on ".$this->date."\n";
	print OUT "\n";
	if (defined $this->license) {
	print OUT "Copyright: Published Under ".$this->license."\n";
	}
	else {
...
sub description {
	...
	$ret.=" .\n" if length $ret;
	if (! $this->without) {
	$ret.=" (Converted from a ".$this->origformat." package by almohawell version $Almohawell::Version.)"
	};
	return $ret;
}
...
sub gen_postinst {
	...
	my $permscript="# almohawell added permissions fixup code\n";
