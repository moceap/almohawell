...
Almohawell::Package::Rpm - an object that represents a rpm package

=cut

package Almohawell::Package::Rpm;
use strict;
...
sub prep {
	...
	print OUT "Requires: ".$this->requires."\n"
		if defined $this->requires && length $this->requires;
	print OUT "Summary: ".$this->summary."\n";
	if (defined $this->license) {
	print OUT "License: ".$this->license."\n";
	}
	else {
	...
	print OUT "\n";
	if (! $this->without) {
	print OUT "(Converted from a ".$this->origformat." package by almohawell version $Almohawell::Version.)\n";
	}
	print OUT "\n";
	print OUT "%files\n";
...
sub _script_helper {
	...
	return "#!/bin/sh\n".
	       "set -e\n".
	       "mkdir /tmp/almohawell.\$\$\n".
	       qq{perl -pe '\$_=unpack("u",\$_)' << '__EOF__' > /tmp/almohawell.\$\$/script\n}.
	       $f."__EOF__\n".
	       "chmod 755 /tmp/almohawell.\$\$/script\n".
	       "/tmp/almohawell.\$\$/script \"\$@\"\n".
	       "rm -f /tmp/almohawell.\$\$/script\n".
	       "rmdir /tmp/almohawell.\$\$";
}
sub postinst {
	my $this=shift;
