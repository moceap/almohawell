...
Almohawell::Package - an object that represents a package

=cut

package Almohawell::Package;
use strict;
...
usable by almohawell. The package may be a deb, a rpm, a tgz, or a slp package,
...
a class derived from this one, such as Almohawell::Package::Rpm. Feed the object
a rpm file, thus populating all of its fields. Then rebless the object into
the destination class, such as Almohawell::Package::Deb. Finally, ask the object
...
sub DESTROY {
	...
	if ($this->unpacked_tree eq '/') {
		die "almohawell internal error: unpacked_tree is set to '/'. Please file a bug report!";
	}
...
=item $Almohawell::Package::verbose
...
$Almohawell::Package::verbose. Returns true if the command succeeds,
false on failure.
...
sub do {
	my $whatever=shift;
	my @command=@_;
	if ($Almohawell::Package::verbose) {
		print "\t@command\n";
	}
	my $pid=fork;
	if (!$pid) {
		# child
		if ($Almohawell::Package::verbose < 2) {
...
This is similar to backticks, but honors $Almohawell::Package::verbose, logging
...
false, errors will abort almohawell.
